...
app.use(async (ctx, next) => {
  ...
  ctx.set('Access-Control-Allow-Headers', 'Content-Type, Content-Length, Authorization, Accept, X-Requested-With , yourHeaderFeild, timestamp');
  ...
  if (ctx.method == 'OPTIONS') {
    ...
    // try {
      await next();   // 执行后代的代码
      console.log(555, ctx.response.connection)
      console.log(ctx)
      if (!ctx.body && ctx.response.connection !== 'keep-alive') {  // 没有资源
        ctx.status = 404;
